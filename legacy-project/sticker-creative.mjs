...
const entries = [
  ...
  ['书房起身看画', '开场直接无遮挡看见人物、书房和已贴好的字画。人物把手中已经合拢的普通尺寸平装书直接平放到身旁桌面，书本始终远离镜头且不翻页、不扬起纸张，随后从阅读椅起身转向字画，镜头跟随视线轻推到画面局部。'],
...
${f.directionNumber === 6 ? '方向6开场连续性强制要求：第0秒直接采用无遮挡的书房中远景，人物、普通尺寸平装书、墙面和完整贴画立即处于正常空间关系中。镜头前方禁止出现白纸、书页、白布、薄膜、幕布或任何大面积白色物体，禁止用掀开、翻开、抽走、滑走、擦镜、遮挡后移开的方式揭示场景或贴画。平装书只能保持合拢状态并由人物在远离镜头的位置直接放到桌面，书页不得朝摄影机扬起、放大或形成转场。' : ''}
