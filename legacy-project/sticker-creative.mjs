...
export function normalizeStickerProfile(profile = {}) {
  ...
  return {
    ...
    frameStructure: '边框为画面印刷的假框，与画身一体柔性弯曲；没有实体边框、木条、挂绳或挂钩',
  };
}
...
const entries = [
  ...
  ['走近欣赏', '客厅全景中人物走近已贴好的画，视线由左向右欣赏，侧让出画面，镜头轻推到产品中景。'],
  ...
  ['轻触成品边缘', '人物指腹轻触已经压实的贴画边缘后移开，不抠起、不揭膜、不再次安装；镜头从人画同框轻推到贴墙边缘。'],
  ...
  ['印刷假框细节', '4—6秒近景沿已经贴实的印刷边框移动至画芯，展示假框仅为平面图案，没有实体厚框、金属反光边或悬空结构。'],
...
材质为柔性PVC薄片，正面为参考图的印刷字画，背面白色，有可揭离的背膜。边框是印刷假框，与画身为同一柔性平面，不是真框。没有木条、挂绳、挂钩、轴头或实体框，不使用小胶带定位。
白色画背和印刷正面属于同一张PVC主体；背膜才是另一个被揭离的物体，不得把白色画背撕成第二张画。背膜按参考素材表现，不确定透明度时不要虚构多层；被揭下的膜由手持有或放到明确可见台面，不能凭空消失。二维字画、印章和假框不能变成真实三维物体，禁止改字、增减笔画、纹理流动。
${f.state === 'installed' ? '本方向为已安装展示：贴画从第0秒就完整压实在同一墙面坐标。全片不揭膜、不拿起、不卷起、不二次展开、不重新安装、不改外形。只有镜头、人物和合理环境微动，画身及四角平整静止。开场未见画只能因取景框外或真实前景遮挡。' : `本方向只执行以下初始状态和动作：${f.action} 未粘区域允许在手支撑下自然弯曲和下垂，不得拉伸或橡胶变形；已粘区域保持固定。贴合只能随手揭膜与压贴逐段推进，已完成后不能再次展开或揭起。演员站地面，不站床、柜子或沙发。`}
...
40个方向中前30个是已安装展示，后10个才涉及柔性形态和安装。以茶室、客厅、书房为主，不默认床头；每条明确地点和2件合理陈设，局部特写例外。产品无挂钩木条挂绳，不使用定位胶带，不套用卷轴尺寸补偿，边框是印刷假框。白色画背属于主体，背膜另行揭离。已贴好不能二次展开。人物正常速度，镜头全程平稳均匀推进，结束焦点在画，允许文字特写结尾。只拍时长内真实可完成的动作。
...
真实住宅自然光、柔和阴影、生活纹理和自然人物，不做卡通、三维渲染或塑料皮肤。声音服从偏好，静音时讲解可只有口型。不添加包装、定位胶带、挂钩、木杆、硬框；禁止二次展开、变形、画面漂移、人物克隆和无操作的物体移动。`;
}
