...
${f.state === 'installed' ? `本方向为已安装展示，以下状态高于创意正文：贴画在第0秒以前已经完成施工，第0秒起整张横画的背胶面与墙面全幅无缝贴合，四角及四边全部压实，画与墙之间不存在空气层或可见间距。正面的画芯、文字、印章、棕色外围印刷线和白色内衬印刷线从产品第一次进入取景框的那一帧起就必须同时、完整、清晰存在，颜色、宽度和相对位置逐帧不变；严禁先出现无边线画芯，再补画、描出、加深、淡入或逐边生成外围印刷线。全片禁止人物手持贴画，禁止拿起、托举、搬运、旋转、横竖转换、卷起、展开、揭膜、贴墙、扶正或再次安装；禁止从竖幅变横幅、从卷材变成品或从小画变大画。只有镜头、人物和合理环境微动，贴画主体逐帧保持同一${p.widthCm}:${p.heightCm}横向外形和同一墙面坐标。开场未见画只能因取景框外或真实前景遮挡；若创意正文出现与此冲突的安装动作，一律删除冲突动作而不是改变产品状态。` : `本方向只执行以下初始状态和动作：${f.action} 未粘区域允许在手支撑下自然弯曲和下垂，不得拉伸或橡胶变形；已粘区域保持固定。贴合只能随手揭膜与压贴逐段推进，已完成后不能再次展开或揭起。演员站地面，不站床、柜子或沙发。`}
