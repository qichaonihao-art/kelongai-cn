...
const entries = [
  ...
  ['书房起身看画', '开场直接采用正面中景，无遮挡看见侧边人物和整幅已贴好的字画；横向贴画从首帧起占9:16画面宽度约55%—65%，棕色外围印刷色带已经完整清楚，不使用全屋大远景。人物把手中已经合拢的普通尺寸平装书直接平放到身旁桌面，书本始终远离镜头且不翻页、不扬起纸张，随后从阅读椅起身转向字画，镜头跟随视线做很短的轻推到画面局部。'],
...
${f.state === 'installed'
    ? '材质为已经完成施工并永久贴实的单层柔性PVC印刷薄片，本片只展示正面完成态。参考图中看似边框的部分只能理解为印在PVC正面的二维装饰边线：装饰边线、画芯和外沿处于同一张连续薄片、同一墙面深度，厚度接近墙贴且不可见，不是真框。禁止生成实体木框、匾框、画框侧壁、背板、内凹画芯、凸起包边、四角拼缝、金属框、玻璃面、框体高光或沿框投到墙上的立体阴影。没有木条、挂绳、挂钩、轴头、实体框或任何施工材料。二维字画、印章和正面装饰边线不能变成真实三维物体，禁止改字、增减笔画、纹理流动。'
...
${f.state === 'installed' ? `本方向为已安装成品展示，以下状态高于创意正文：第0秒起墙上已经存在最终完成态的整张横画，整个表面与墙面全幅无缝贴合，四角及四边全部压实，二者之间没有空气层或可见间距。正面的画芯、文字、印章、棕色外围印刷线和白色内衬印刷线从产品第一次进入取景框的那一帧起就必须同时、完整、清晰存在，颜色、宽度和相对位置逐帧不变；严禁先出现无边线画芯，再补画、描出、加深、淡入或逐边生成外围印刷线。人物始终空手并与产品表面保持距离，产品全片都是同一个贴墙静态平面；唯一变化来自镜头、人物和合理环境微动。贴画主体逐帧保持同一${p.widthCm}:${p.heightCm}横向外形和同一墙面坐标，首帧状态就是末帧状态。` : `本方向只执行以下初始状态和动作：${f.action} 未粘区域允许在手支撑下自然弯曲和下垂，不得拉伸或橡胶变形；已粘区域保持固定。贴合只能随手揭膜与压贴逐段推进，已完成后不能再次展开或揭起。演员站地面，不站床、柜子或沙发。`}
${f.directionNumber === 6 ? '方向6开场连续性强制要求：第0秒直接采用无遮挡的正面中景，人物、普通尺寸平装书、墙面和完整贴画立即处于正常空间关系中；完整横向贴画从首帧起占9:16视频画面宽度约55%—65%，外围棕色二维印刷色带在首帧已有足够像素完整清楚呈现。禁止全屋大远景，禁止把贴画缩在画面远处后再依靠推近补出边线。镜头前方禁止出现白纸、书页、白布、薄膜、幕布或任何大面积白色物体，禁止用掀开、翻开、抽走、滑走、擦镜、遮挡后移开的方式揭示场景或贴画。平装书只能保持合拢状态并由人物在远离镜头的位置直接放到桌面，书页不得朝摄影机扬起、放大或形成转场。' : ''}
...
40个方向中前30个是已安装展示，后10个才涉及柔性形态和安装。前30个方向的贴画必须在第0秒以前已经贴好，创意不得给它增加手持、搬运、展开、旋转或贴墙动作。每个方向必须服从该方向指定的唯一功能空间及家具，不跨场景混搭；茶室、书房、餐厅、办公室、走廊和工作区不得自动补入住宅沙发或成排靠垫，只有明确标为客厅的方向才允许使用该方向指定的沙发。每条明确地点和2件符合该空间用途的陈设，局部特写例外。产品无挂钩木条挂绳，不使用定位胶带，不套用卷轴尺寸补偿；所谓边框只能是印在PVC正面的平面装饰边线，不能描述为框体、相框或匾。成品与本方向主家具组合对应的功能墙几何中心对齐。白色画背属于主体，背膜另行揭离。已贴好不能二次展开。人物正常速度，镜头全程平稳均匀推进，结束焦点在画，允许文字特写结尾。只拍时长内真实可完成的动作。
...
真实住宅自然光、柔和阴影、生活纹理和自然人物，不做卡通、三维渲染或塑料皮肤。声音服从偏好，静音时讲解可只有口型。不添加包装、定位胶带、挂钩、木杆、硬框、背板或玻璃；禁止把印刷装饰边线变成立体匾框，禁止二次展开、横竖旋转、变形、画面漂移、人物克隆和无操作的物体移动。${f.state === 'installed' ? '这是纯成品展示，输出文本中完全不要描述产品背侧、施工材料或任何剥离过程，只描述已经贴实的正面成品。' : ''}已安装方向的最终提示词若出现“手持画、搬画、展开画、旋转画、把画贴上墙”等动作，必须在输出前删除这些动作。`;
}
