...
assert.equal(calls[0].url, '/api/sticker/analyze');
...
const ideasRequestCall = calls.find((call) => call.url === '/api/sticker/ideas')!;
...
assert.equal(ideasRequest.productType, 'sticker');
await generatePaintingIdeaPrompt(profile, idea, plan);
const promptRequestCall = calls.find((call) => call.url === '/api/sticker/idea-prompt')!;
